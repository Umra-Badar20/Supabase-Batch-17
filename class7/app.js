...
try{
  ...
  if(error || !data){
    alert("You must Login first to view you own post")
    return
  }
  console.log(data);
  
  const {data : postData , error: postError} = await supabase.from("post_duplicate").select("*").eq("user_id",data.user.id).order("id", { ascending: false });
  if(postError){
